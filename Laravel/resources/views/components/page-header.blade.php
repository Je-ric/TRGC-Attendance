    ...
    <div class="flex items-start md:items-center gap-3 sm:gap-4">
        @if($icon)
            <span class="shrink-0 inline-flex items-center justify-center rounded-xl text-white w-10 h-10 sm:w-11 sm:h-11 text-xl
                         bg-gradient-to-br from-[#93291e] to-[#ed213a]"
                  style="box-shadow: 0 2px 8px rgba(237,33,58,0.25)">
                <i class="bx {{ $icon }}"></i>
            </span>
        @endif
        <div>
            <h1 class="font-['Oswald'] text-[15px] sm:text-[18px] font-bold text-[#1c1c1e] leading-snug">{{ $title }}</h1>
            @if($desc)

...
<div class="flex flex-col gap-6">
    ...
    <x-page-header icon="bx-home-alt-2" title="Dashboard"
        desc="Overview of congregation attendance and activity — {{ now()->format('l, F j, Y') }}.">
    ...
    <div class="grid gap-4 lg:grid-cols-3">
        ...
        <x-card color="red" :padding="false">
            <div class="px-4 py-3 border-b border-[#e4e0e2] bg-[#fff0f0]">
                <div class="page-eyebrow mb-1">Most Recent</div>
                <h3 class="page-title text-[16px]">Last Session</h3>
            </div>
            <div class="p-4">
                @if($lastSession)
                    <div class="flex items-start justify-between gap-3 mb-4">
                        <div>
                            <div class="text-[15px] font-bold text-[#1c1c1e]" style="font-family:'Oswald',sans-serif">
                                {{ $lastSession->attendanceType?->name ?? '—' }}
                            </div>
                            @if($lastSession->service_name)
                                <div class="text-[12px] text-[#6b6570] mt-0.5">{{ $lastSession->service_name }}</div>
                            @endif
                            <div class="text-[12px] text-[#a09aa4] mt-1 flex items-center gap-1">
                                <i class='bx bx-calendar text-[11px]'></i>
                                {{ $lastSession->date->format('l, F j, Y') }}
                            </div>
                        </div>
                        <div class="text-right shrink-0">
                            <div class="text-[36px] font-bold text-[#ed213a] leading-none" style="font-family:'Oswald',sans-serif">
                                {{ $lastSessionCount }}
                            </div>
                            <div class="text-[11px] font-bold uppercase tracking-[0.1em] text-[#a09aa4]">present</div>
                        </div>
                    </div>

                    {{-- Category breakdown of last session --}}
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="h-1.5 rounded-full bg-[#ed213a]"
                                             style="width:{{ max(8, ($count / $lastSessionCount) * 80) }}px"></div>
                                        <span class="text-[12px] font-semibold text-[#1c1c1e] w-6 text-right">{{ $count }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    @foreach($recentSessions as $s)
                        ...
                        <div class="flex-1 flex flex-col items-center gap-1 group">
                            ...
                            <div class="w-full rounded-t-md transition-all duration-300"
                                 style="height:{{ $pct }}%;background:linear-gradient(180deg,#ed213a,#93291e);min-height:4px"></div>
                        ...
                        <div class="flex items-center gap-3 px-4 py-3 hover:bg-[#fff5f5] transition-colors">
                            ...
                            <div class="w-6 text-center shrink-0">
                                ...
                                @elseif($i === 2)
                                    <span class="text-[16px]">🥉</span>
                                @else
                                    <span class="text-[12px] font-bold text-[#a09aa4]">{{ $i + 1 }}</span>
                                @endif
                            </div>
                            {{-- Avatar --}}
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-[13px] font-semibold text-[#1c1c1e] truncate">{{ $summary->person->full_name }}</div>
                            ...
                            {{-- Rate --}}
                            <div class="text-right shrink-0">
                                <div class="text-[15px] font-bold text-[#ed213a]" style="font-family:'Oswald',sans-serif">
                        ...
                        @foreach($streakLeaders as $summary)
                            <div class="flex items-center justify-between gap-3">
                                ...
                                <div class="flex items-center gap-1 shrink-0">
                                    <span class="text-[15px] font-bold text-[#ed213a]" style="font-family:'Oswald',sans-serif">
                                        {{ $summary->streak }}
                                    </span>
                        ...
                        @foreach($upcomingBirthdays as $person)
                            <div class="flex items-center justify-between gap-3">
                                <div class="flex items-center gap-2 min-w-0">
                                    ...
                                    <div class="min-w-0">
                                        <div class="text-[13px] font-medium text-[#1c1c1e] truncate">{{ $person->full_name }}</div>
                                        <div class="text-[11px] text-[#a09aa4]">
                    ...
                    <x-table.body>
                        @foreach($inactiveMembers as $person)
                            ...
                            <x-table.row :hover="true">
                                ...
                                <x-table.td>
                                    @if($lastSeen)
                                        <div class="text-[13px] text-[#1c1c1e]">{{ $lastSeen->format('M d, Y') }}</div>
                                        <div class="text-[11px] text-[#a09aa4]">{{ $daysAgo }} days ago</div>
                                    @else
                                        <span class="text-[#a09aa4] italic text-[12px]">Never recorded</span>
                                    @endif
                                </x-table.td>
                                <x-table.td align="center">
                                    @if($summary)

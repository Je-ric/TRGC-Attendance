...
@php
    ...
    $membershipColor = $membershipColors[$person->membership_status ?? ''] ?? 'slate';
@endphp

<div class="flex items-center justify-between gap-3 py-2.5 {{ $compact ? '' : 'px-1' }}">
    <div class="flex items-center gap-3 min-w-0">
        {{-- Avatar --}}
        <div class="w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-white text-[12px] font-bold"
        ...
        <div class="min-w-0">
            ...
            <div class="flex items-center gap-1.5 flex-wrap mt-0.5">
                ...
                @if($person->membership_status && $person->membership_status !== 'Regular Attendee')
                    <x-feedback-status.status-indicator :variant="$membershipColor">{{ $person->membership_status }}</x-feedback-status.status-indicator>
                @endif
                @if($person->age) <span class="text-[11px] text-[#a09aa4]">{{ $person->age }} yrs</span> @endif
                @if($showFamily && $person->family) <span class="text-[11px] text-[#a09aa4]">· {{ $person->family->family_name }}</span> @endif
            </div>
        </div>
    </div>
